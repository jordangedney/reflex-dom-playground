...
module Frontend.Page.PixelVines.App where
...
-- tested 1-20, but only three does it. Weird, right?
magicNumber :: Int
magicNumber = 3
...
toVisit inbounds visited frontier (r:randomIndexes) = toVisit'
  where (nextNode, frontierWithout) = popRandomNode r frontier
        ...
        toVisit' =
          case newFrontier of
            [] -> [nextNode]
            _ -> nextNode:(toVisit inbounds
                                  (nextNode:visited)
                                  newFrontier (drop magicNumber randomIndexes))
